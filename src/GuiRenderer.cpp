...
namespace sf
{
    ...
    void    GuiRenderer::InjectEvent(const Event& event)
    {
        ...
        if (Widget::mFocusedWidget)
        {
            ...
            newEvent.MouseMove.X = input.GetMouseX();
            newEvent.MouseMove.Y = input.GetMouseY();

            // We have to simulate the MouseEntered and MouseLeft events for Widgets.
            if (event.Type == Event::MouseMoved)
            {
                bool inside = Widget::mFocusedWidget->GetRect(true).Contains(newEvent.MouseMove.X, newEvent.MouseMove.Y);
    ...
    void    GuiRenderer::Resize(const Vector2<unsigned int>& size)
    {
    ...
    bool    GuiRenderer::SetFocusUnderMouse(Widget* widget, const Vector2<unsigned int>& mouse, Vector2f absPos)
    {
